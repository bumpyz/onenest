// PKCE callback for the direct Google OAuth flow. Mirrors src/app/oauth/microsoft.tsx.
//
// Lifecycle:
//   1. User clicked "Connect Google" in Settings → /oauth/google?code=...&state=...
//   2. We pull the stashed PKCE verifier + CSRF state from sessionStorage
//   3. Verify state matches (CSRF protection)
//   4. Exchange code → tokens through the google-oauth-proxy edge function (which adds
//      the client_secret server-side)
//   5. Fetch the connected Google account's email to label the pairing
//   6. Persist via saveGoogleCalendarPairing (which writes through Vault per migration 0017)
//   7. Kick off an initial sync (failures here are non-fatal — user can retry from Settings)
...
import { saveGoogleCalendarPairing } from '@/lib/db';
...
export default function GoogleOAuthCallback() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            const stored = consumeGoogleOAuthState();
            if (!stored) {
                setStatus('error');
                setMessage(
            ...
            try {
                ...
                const pairing = await saveGoogleCalendarPairing({
                    email,
                    accessToken: token.access_token,
                    refreshToken: token.refresh_token ?? null,
                    expiresAt,
                });
                // Best-effort initial sync — failure isn't fatal, user can retry from Settings.
                try {
                    await syncGoogleCalendar(pairing);
                } catch (syncErr) {
                    console.warn('Initial Google sync failed', syncErr);
                }
                if (cancelled) return;
                setStatus('ok');
