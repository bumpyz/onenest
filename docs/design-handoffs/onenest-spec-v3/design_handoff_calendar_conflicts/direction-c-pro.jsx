        ...
        <div style={{ position: 'absolute', inset: 0, paddingTop: 54, paddingBottom: 80 }}>
          ...
          <div style={{ position: 'absolute', top: 220, left: 0, right: 0, bottom: 80, overflow: 'hidden' }}>
            <div style={{ position: 'absolute', inset: '0 12px', overflow: 'auto' }}>
              <div style={{ position: 'relative', minHeight: '100%', background: C.card, borderRadius: 10, border: `0.5px solid ${C.hair}`, paddingBottom: 8 }}>
                {hours.map((h, i) => (
                  <div key={i} style={{
          ...
          {/* Floating drag-hint footer — TRANSIENT. Only visible when the user is
              actively dragging an event block to a new slot. Static mock
              normalized to the default state; see this section's commentary in
              the docs for the drag-state mock. */}
          {/* (removed from default render) */}
        </div>

        {/* FAB — sits at the canonical position. During an active drag the FAB
            should fade out (the user's intent is to move, not create) and the
            drag-hint banner takes its place. */}
        <div style={{
          position: 'absolute', right: 16, bottom: 96,
          height: 44, padding: '0 16px', borderRadius: 22, background: C.accent,
          display: 'flex', alignItems: 'center', gap: 8,
...
function CCalBlock({ day, startH, endH, color, title, conflict, ghost, handoff, right }) {
  ...
  return (
    <div style={{
      ...
      {/* Conflict tap-target — dedicated affordance in the corner. Block itself
          still opens EventDetail; this dot opens ConflictResolution directly. */}
      {conflict && !ghost && (
        <div style={{
          position: 'absolute', right: 2, bottom: 2,
          width: 14, height: 14, borderRadius: 7,
          background: C.warn, border: `1.5px solid ${C.card}`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          boxShadow: '0 1px 2px rgba(0,0,0,0.15)',
        }}>
          <svg width="7" height="7" viewBox="0 0 8 8" fill="none">
            <path d="M4 1.5v3M4 6v.4" stroke="#fff" strokeWidth="1.4" strokeLinecap="round"/>
          </svg>
        </div>
      )}
    </div>
