...
ReactDOM.createRoot(document.getElementById('root')).render(
  <DesignCanvas>
    ...
    <DCSection id="pair-strip-caregiver" title="02.1.A · Custody strip — caregiver viewer (#397)" subtitle="The Today strip reshaped for caregivers (nannies, grandparents, etc). Observer framing — caregiver isn't a party to the hand-off. Read-only with explicit VIEWING badge. Five mockup states stacked in the showcase: ① default, ② hand-off day, ③ countdown active (brief the next parent), ④ both parents on duty (overlap day), ⑤ overflow with long names.">
      <DCArtboard id="p3-strip-caregiver"          label="P3 · showcase"   width={402} height={874}><CaregiverShowcase palette={paletteMistForest} /></DCArtboard>
      <DCArtboard id="p3-strip-caregiver-context"  label="P3 · in context" width={402} height={874}><CaregiverInContext palette={paletteMistForest} /></DCArtboard>
      <DCArtboard id="p4f-strip-caregiver"         label="P4-F · showcase" width={402} height={874}><CaregiverShowcase palette={paletteCharcoalForest} /></DCArtboard>
    </DCSection>

    <DCSection id="pair-strip-external" title="02.1.B · Custody strip — external co-parent (#398)" subtitle="The Today strip reshaped for external co-parents (parents outside the household who share custody of one or more kids in it). Header anchors to the KID, not the household's week. Five mockup states: ① default (your kid is with the household, comes to you Fri), ② hand-off day, ③ countdown active, ④ multi-kid (two stacked strips when sharing 2+ kids), ⑤ overflow + paired-calendar busy-block overlay.">
      <DCArtboard id="p3-strip-external"           label="P3 · showcase"   width={402} height={874}><ExternalShowcase palette={paletteMistForest} /></DCArtboard>
      <DCArtboard id="p3-strip-external-context"   label="P3 · in context" width={402} height={874}><ExternalInContext palette={paletteMistForest} /></DCArtboard>
      <DCArtboard id="p4f-strip-external"          label="P4-F · showcase" width={402} height={874}><ExternalShowcase palette={paletteCharcoalForest} /></DCArtboard>
    </DCSection>
    ...
    {/* ───────────────────────────────────────────────────────────────────
        05 · Lists & tasks
        ─────────────────────────────────────────────────────────────── */}
    <DCSection id="pair-lists" title="05.1 · Lists overview (current)" subtitle="Lists tab with FAB + tappable 'Your lists' cards row that navigates to List detail. Filter chips below stay as filters (scope the task stream). Two affordances, two intents: cards = navigation, chips = filter.">
      <DCArtboard id="p3-lists"   label="P3 · light"  width={402} height={874}><ProListsV2 palette={paletteMistForest} /></DCArtboard>
      <DCArtboard id="p4f-lists"  label="P4-F · dark" width={402} height={874}><ProListsV2 palette={paletteCharcoalForest} /></DCArtboard>
    </DCSection>

    <DCSection id="pair-lists-v1" title="05.1.b · Lists overview — reference (no FAB, no list cards)" subtitle="Original Lists screen. Kept for diff reference — superseded by 05.1.">
      <DCArtboard id="p3-lists-v1"   label="P3 · light"  width={402} height={874}><ProLists palette={paletteMistForest} /></DCArtboard>
      <DCArtboard id="p4f-lists-v1"  label="P4-F · dark" width={402} height={874}><ProLists palette={paletteCharcoalForest} /></DCArtboard>
    </DCSection>

    <DCSection id="pair-list-detail" title="05.2 · List detail" subtitle="Open the Grocery list · color-tinted header, progress bar, filtered task groups (Today / This week / Done), inline qty + priority chips, list metadata + sharing. FAB labeled 'New task' focuses the inline quick-add row at the top (doesn't open a separate sheet) — fast-entry by default, with a 'More fields →' escalation link to the full CreateTask form when needed.">
      <DCArtboard id="p3-list-detail"          label="P3 · light"           width={402} height={874}><ListDetail palette={paletteMistForest} /></DCArtboard>
      <DCArtboard id="p3-list-detail-focused"  label="P3 · adding item"     width={402} height={874}><ListDetail palette={paletteMistForest} focused={true} /></DCArtboard>
